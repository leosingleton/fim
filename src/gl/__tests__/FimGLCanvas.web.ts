  ...
  return () => {
    it('Creates and disposes', () => {
      using(new FimWeb(canvasFactory), fim => {
        const b = fim.createGLCanvas(640, 480);
    ...
    it('Renders a JPEG texture', async () => {
      await DisposableSet.usingAsync(async disposable => {
        ...
        const canvas = disposable.addDisposable(fim.createGLCanvas(128, 128));
    ...
    it('Downscales oversized canvases', async () => {
      await DisposableSet.usingAsync(async disposable => {
        ...
        const texture = disposable.addDisposable(gl.createTexture(canvasSize, canvasSize / 8));
        const jpeg = FimTestImages.fourSquaresJpeg();
        const buffer = disposable.addDisposable(await fim.createCanvasFromJpegAsync(jpeg));
        ...
        const left = canvasSize / 4;
        const right = canvasSize * 3 / 4;
        const top = canvasSize / 32;
        const bottom = canvasSize * 3 / 32;
